...
const repo = inMemoryRepo<NewAirline>([])

export const airlinesMockService: AirlinesServicesInterface = {
  async changeAirline(params: { update: Airline; id: AirlineId }) {
    const { found, item: airline } = await repo.update(params.id, params.update)
