...
async function main() {
  const data = `IATA_CODE,AIRPORT,CITY,STATE,COUNTRY,LATITUDE,LONGITUDE
...
YUM,Yuma International Airport,Yuma,AZ,USA,32.65658,-114.60597`
    ...
    .slice(1)
    .map((x) => {
      const [iataCode, airport, city, state, country, latitude, longitude] =
        x.split(",")

      const l = parseFloat(latitude || "0")
      const lo = parseFloat(longitude || "0")
